...
const sendVerificationEmail = async (email, verificationCode) => {
  try {
    ...
    const logoUrl = process.env.NODE_ENV === 'production' 
      ? `${process.env.FRONTEND_URL || 'https://virellofoods.com'}/images/logo.svg`
      : `${process.env.FRONTEND_URL || 'http://localhost:3000'}/images/logo.svg`;
    ...
    try {
      ...
      const possiblePaths = [
        path.join(__dirname, '../../frontend/public/images/logo.svg'), // From lib folder
        path.join(__dirname, '../frontend/public/images/logo.svg'),     // Alternative path
        path.join(process.cwd(), 'frontend/public/images/logo.svg'),    // From project root
        path.join(__dirname, '../public/images/logo.svg')              // If logo is in backend public
      ];
