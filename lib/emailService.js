...
const sendEmailWithResend = async (email, subject, htmlContent, textContent, attachments = []) => {
  try {
    if (!resend) {
      console.log('⚠️ Resend client not initialized - API key not configured');
      return { success: false, error: 'Resend API key not configured', service: 'resend' };
    }
    
    console.log('📧 Attempting to send email via Resend...');
    
    const emailData = {
      from: process.env.RESEND_FROM_EMAIL || process.env.EMAIL_FROM || 'Virello Food <noreply@virellofoods.com>',
      to: [email],
      subject: subject,
      html: htmlContent,
      text: textContent
    ...
    return { success: false, error: error.message, service: 'resend' };
  }
};

// Enhanced email sending with fallback to Resend
const sendEmailWithFallback = async (transporter, mailOptions, maxRetries = 3) => {
  // First try the primary email service
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      console.log(`📤 Attempt ${attempt}/${maxRetries} - Sending email via primary service...`);
      ...
      console.log(`✅ Email sent successfully via primary service on attempt ${attempt}!`);
      console.log('📧 Message ID:', result.messageId);
      console.log('📧 Response:', result.response);
      return { success: true, messageId: result.messageId, attempt, service: 'primary' };
      
    } catch (error) {
      console.error(`❌ Attempt ${attempt}/${maxRetries} failed:`, error.message);
      
      if (attempt === maxRetries) {
        console.log('🔄 Primary service failed, attempting fallback to Resend...');
        ...
        if (resendResult.success) {
          ...
          console.log('❌ Both primary service and Resend failed');
...
const sendVerificationEmail = async (email, verificationCode) => {
  try {
    ...
    console.log('📤 Sending email with fallback mechanism...');
    const result = await sendEmailWithFallback(transporter, mailOptions);
...
const sendPasswordResetEmail = async (email, resetToken) => {
  try {
    ...
    console.log('📤 Sending password reset email with fallback mechanism...');
    const result = await sendEmailWithFallback(transporter, mailOptions);
...
const listResendApiKeys = async () => {
  try {
    ...
    return { success: true, keys: keys.data };
  } catch (error) {
...
const createResendApiKey = async (name, permission = 'full_access') => {
  try {
    if (!resend) {
      return { success: false, error: 'Resend client not initialized' };
    }
...
const removeResendApiKey = async (keyId) => {
  try {
    if (!resend) {
      return { success: false, error: 'Resend client not initialized' };
    }
